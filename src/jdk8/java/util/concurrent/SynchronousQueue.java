...
public class SynchronousQueue<E> extends AbstractQueue<E>
    ...
    static final class TransferStack<E> extends Transferer<E> { // 以栈的方式实现的Transfer
        ...
        static final int REQUEST    = 0; // 栈中节点类型 消费者 请求数据
        /** Node represents an unfulfilled producer */
        static final int DATA       = 1; // 栈中节点类型 生产者 提供数据
        /** Node is fulfilling another unfulfilled DATA or REQUEST */
        static final int FULFILLING = 2; // 栈中节点类型 二者正在匹配中
        ...
        static final class SNode { // 栈中的节点
            ...
            volatile SNode match;       // the node matched to this // 匹配者
            volatile Thread waiter;     // to control park/unpark // 等待这的线程
            Object item;                // data; or null for REQUESTs // 元素
            int mode; // 模式 也就是节点的类型：消费者、生产者、匹配中
             ...
             */
            boolean tryMatch(SNode s) { // SNode里面的方向 调用者m是s的下一个节点 这时候m节点的线程应该是阻塞状态
                if (match == null &&
                ...
                return match == s; // 可能其他线程先一步匹配了m 返回其是否是s
            }
         ...
         */
        static SNode snode(SNode s, Object e, SNode next, int mode) { // put场景：s=null e=元素 next=null mode=REQUEST
        ...
        E transfer(E e, boolean timed, long nanos) { // 从put方法进来的时候e是要生产者表示要放的元素 从take方法进来的时候e是null表示要取元素 put场景：put方法进来 take场景：take方法进来
            ...
            for (;;) { // 自旋+cas
                ...
                if (h == null || h.mode == mode) {  // empty or same-mode // 栈顶没有元素 或者栈顶元素模式跟当前元素模式是一样的(也就是都是生产者节点或者都是消费者节点 上一次是put这次也是put 上次是take这次也是take)
                    ...
                    } else if (casHead(h, s = snode(s, e, h, mode))) { // 因为模式相同 只能入栈 如果入栈成功 s=SNode(next=h,match=null,waiter=null,item=e,mode=mode) 如果头节点h为空就把头节点换成s
                        SNode m = awaitFulfill(s, timed, nanos); // 调用awaitFulfill()方法自旋+阻塞当前入栈的线程并等待被匹配到
                        if (m == s) {               // wait was cancelled // 如果m等于s 说明取消了 那么就把它清除 并返回null
                            clean(s);
                            return null; // 被取消了返回null
                        } // 代码走到这里 说明匹配到元素了 因为从awaitFulfill()方法里面出来要么被取消了 要么就匹配到了
                ...
                } else if (!isFulfilling(h.mode)) { // try to fulfill // 代码到这说明头节点和当前节点的模式不一样(头节点是put当前是take) 如果头节点不是正在匹配中
                    if (h.isCancelled())            // already cancelled // 如果头节点已经取消了 就把它弹出栈
                        ...
                        for (;;) { // loop until matched or waiters disappear
                            ...
                            if (m.tryMatch(s)) { // 如果m和s尝试匹配成功 就弹出栈顶的两个元素m和s
                                ...
                                return (E) ((mode == REQUEST) ? m.item : s.item); // 返回匹配的结果
                            } else                  // lost match
                                s.casNext(m, mn);   // help unlink // 尝试匹配失败 说明m已经先一步被其他线程匹配了 就协助清除它
                        }
                    }
        ...
        SNode awaitFulfill(SNode s, boolean timed, long nanos) {
            ...
            Thread w = Thread.currentThread(); // 当前线程w
            int spins = (shouldSpin(s) ?
                         (timed ? maxTimedSpins : maxUntimedSpins) : 0); // 自旋次数 0或者32或者512
            for (;;) { // 自旋
        ...
        // Unsafe mechanics
        private static final sun.misc.Unsafe UNSAFE;
        private static final long headOffset; // TransferStack中属性head在TransferStack对象中内存地址偏移量
        static {
            try {
                UNSAFE = sun.misc.Unsafe.getUnsafe();
    ...
    /** Dual Queue */
    static final class TransferQueue<E> extends Transferer<E> { // 以队列方式实现的Transfer
        /*
        ...
        static final class QNode { // 队列中的节点
            volatile QNode next;          // next node in queue // 下一个节点
            volatile Object item;         // CAS'ed to or from null // 存储的元素
     ...
     */
    private transient volatile Transferer<E> transferer; // 传输器 两个线程之间交换元素使用
