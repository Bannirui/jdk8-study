 ...
 * @author Doug Lea
 */
public interface ExecutorService extends Executor { // 线程池次级接口，对Executor做了一些扩展，主要增加了关闭线程池、执行有返回值任务、批量执行任务的方法
     ...
     *         denies access.
     */
    List<Runnable> shutdownNow(); // 立即关闭线程池，尝试停止正在运行的任务，未执行的任务将不再执行 被迫停止及未执行的任务将以列表的形式返回
     ...
     * @throws NullPointerException if the task is null
     */
    Future<?> submit(Runnable task); // 执行有返回值的任务，任务的返回值为null 当然只有当任务执行完成了调用get()时才会返回
